"""Basic statistics of graph structures."""
...
def _get_ring_key(list_of_nodes):
    """
...
def count_rings(graph: nx.DiGraph, ring_size: int , node: int = -1) -> int:
    """Find the number of rings associated with a node via depth-first-search.
    
    
    Args:
        graph: Input graph to count rings in. Expects a directed graph of only water clusters
        node: Count number of rings attached to this node. 
            Count rings in whole graph is node is set to -1.
        ring_size: Size of the ring, would be 3 for a trimer.
    Returns:
        ...
        for nbr in graph.neighbors(next_node):
            ...
            if curr_path_len == ring_size:
                if nbr == path_to_expand[0]:
                    ...
                    ring_key = _get_ring_key(ring_path)
    ...
    # Upon termination 
    #print('%d rings of size %d' % (len(rings), ring_size)) 
    #for r in rings:
        #print(r)
    return len(rings)
