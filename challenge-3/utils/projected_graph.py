...
def project_oxygen_role_based_graph(graph, edge_weight=2.90):
    """
    ...
    for edge, count in weighted_edges.items():
        projected_graph.add_edge(edge[0], edge[1], label='link', weight=count)

    attrs=nx.get_edge_attributes(projected_graph,'weight')

    for key in attrs:    
        attrs[key]={"length": edge_weight * attrs[key]}

    nx.set_edge_attributes(projected_graph, attrs)

    return projected_graph

def directed_coarse_graph(graph, edge_weight=2.90):
    node_roles, role_distribution = compute_analytics.molecular_role_distribution(graph)
    weighted_edges = graph_projection(graph)
