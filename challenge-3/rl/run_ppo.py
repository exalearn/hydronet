...
# TODO (wardlt): Do we need this? Could we install the library with `pip install -e` instead?
import sys
...
if __name__ == "__main__":
    ...
    out_dir = Path(f'runs/{args.reward}-W{args.max_size}-{datetime.utcnow().strftime("%y%m%d_%H%M%S")}-{run_hash}')
    ...
    log_driver = DriverLogger(out_dir / 'training-structures.json')
    driver = DynamicEpisodeDriver(tf_env, tf_agent.collect_policy, [buffer.add_batch, log_driver], num_episodes=8)
    ...
    with open(out_dir / 'critic_network.pkl', 'wb') as fp:
        pkl.dump(critic_net, fp)
    with open(out_dir / 'actor_network.pkl', 'wb') as fp:
        pkl.dump(actor_net, fp)
